...
async def request_password_reset(db: AsyncSession, email: str) -> bool:
    ...
    try:
        ...
        # ponytail: swallow + log. Never let a provider error (403 test-mode,
        # rate limit, outage) become an unhandled 500 — that also strips CORS
        # headers. And never reveal send status to the client (anti-enumeration).
        # Real failures surface in Cloud Run logs, not the API response.
        logger.error("Password reset email failed for user %s: %s", user.id, e)
    return True
